Git Repository der Gruppe Skyware für die Vorlesung NOSQL

Um unsere Software zu starten sind folgende Schritte notwendig:

1) Projekt downloaden

2) Mongoserver mit folgendem Argument starten: mongod --port 27017 --dbpath="Pfad zum Ordner /data in unserem Projekt"

3) Die Applikation mit dem Befehl node app.js starten - Nun läuft der Server auf dem Port 3000

4) Sobald der Server gestartet ist, kann man die Website aufrufen. Unter dem Reiter Settings kann man 
    nun Teams und danach Spiele hinzufügen.
